...
def plot_strategy(ohlcv_data, signals):
    ...
    if 'ema_fast' in df.columns: plt.plot(df.index, df['ema_fast'], label='EMA Fast', color='orange')
    if 'ema_slow' in df.columns: plt.plot(df.index, df['ema_slow'], label='EMA Slow', color='red')
